...
class Test(unittest.TestCase):
    ...
    def test_lds(self):
        ...
        self.assertEqual(
            np.around(0.10483034490920523, 1),
            np.around(
                orbital_to_spin_ratio(
                    xmcd=testxmcd, group=None, xp=2750, orbital="3d"
                ),
                1,
            ),
        )
        ...
        # test for group:
        orbital_to_spin_ratio(
            xmcd=None, group=testgroup, xp=2750, orbital="3d"
        )
        self.assertEqual(
            np.around(0.10483034490920523, 1), np.around(testgroup.lds, 1)
        )
